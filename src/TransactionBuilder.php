...
class TransactionBuilder
{
    ...
    public function delegateresource(string $receiver_address, float $amount = 0, string $resource = 'ENERGY', string $owner_address = null, $lock = false, $lock_period = 3)
    {
        if (empty($owner_address))
            throw new TronException('owner_address not specified');
        if (empty($receiver_address))
            throw new TronException('receiver_address not specified');
        ...
        if ($lock && $lock_period === null) {
            throw new TronException('If lock is true, lock_period must be an integer.');
        }
    ...
    public function unDelegateResource(string $receiver_address, float $amount = 0, string $resource = 'ENERGY', string $owner_address = null)
    {
        if (empty($owner_address))
            throw new TronException('owner_address not specified');
        if (empty($receiver_address))
