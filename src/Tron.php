...
class Tron implements TronInterface
{
     ...
     * @throws TronException
     */
    public function delegateResource(string $receiver_address, float $amount, string $resource = 'ENERGY', string $owner_address = null, bool $lock = false, $lock_period = 3)
    {
        if ($owner_address == null) {
            $owner_address = $this->address['hex'];
        }

        $resource = $this->transactionBuilder->delegateresource($receiver_address, $amount, $resource, $owner_address, $lock, $lock_period);
        $signedTransaction = $this->signTransaction($resource);
        $response = $this->sendRawTransaction($signedTransaction);

        return array_merge($response, $signedTransaction);
    }

    public function unDelegateResource(string $receiver_address, float $amount, string $owner_address = null, string $resource = 'ENERGY'): array
    {
        if ($owner_address == null) {
            $owner_address = $this->address['hex'];
        }

        $receiver = $this->transactionBuilder->unDelegateResource($receiver_address, $amount, $resource, $owner_address);
        $signedTransaction = $this->signTransaction($receiver);
        $response = $this->sendRawTransaction($signedTransaction);

        return array_merge($response, $signedTransaction);
    }

    public function getcandelegatedmaxsize(int $type = 0, string $owner_address = null, $fromTron = true): float
    {
        //0 is bandwidth, 1 is energy
        if (!in_array($type, [0, 1])) {
            throw new TronException('Invalid type,only 0 or 1');
        }
        $address = (!is_null($owner_address) ? $owner_address : $this->address['hex']);
        $result = $this->manager->request('/wallet/getcandelegatedmaxsize', [
            'owner_address' => $this->address2HexString($address),
            'type' => $type,
        ]);
        $first = array_shift($result);
        return ($fromTron == true ? $this->fromTron($first) : $first);
    }
}
